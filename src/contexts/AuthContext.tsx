...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const updateJackPoints = async (amount: number, description: string, type: Transaction['type']): Promise<Transaction | null> => {
    ...
    try {
      ...
      const transaction = createTransaction(amount, description, type);
      
      // Check if this is a registration/signup bonus
      const isSignupBonus = (
        description.includes('Üyelik Tamamlama Bonusu') || 
        description.includes('Kayıt Bonusu') ||
        description.includes('Profil Oluşturma Bonusu')
      );
      ...
      const updatedUser: User = {
        ...
        lastUpdated: Date.now(),
        // If this is a signup bonus, mark that they've received it
        hasReceivedInitialBonus: user.hasReceivedInitialBonus || isSignupBonus,
        rank: user.rank,
      ...
      console.error('Failed to update jack points:', err);
      setError('Failed to update jack points');
      return null;
    }
  };
